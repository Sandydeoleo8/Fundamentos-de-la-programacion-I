...
//Observa que esta funcion va a recibir un archivo como paranetro.
...
//Esta funcion cunenta el numero de minusculas y mayusculas que hay en el archivo arc.
{
    int min = 0, may = 0;
    char p;
    while (!feof(arc))
    {
        p = fgetc(arc);         //Se utiliza la funcion fgetc() para leer caracter del archivo.
        if (islower(p))
            min++;
            else
            if (isupper(p))
