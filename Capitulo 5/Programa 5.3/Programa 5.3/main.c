...
void main(void)
{
    ...
    *IX = 15;           // Z[2] se modifica, ahora vale 15.
    ...
    IX = *IX + 5;       // Se modifica, ahora vale Z[2] + 5 = 20. Recuerda que *IX contiene en valor de Z[2].
    Y = *IX - 5;        // Z[2] se modifica, ahora vale 10.
    printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z [4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
    ...
    *IX += 1;           // Z[2] se vuelve a modificar, ahora vale 12.
    ...
    IX = IX + 1;        // IX se modifica. Obseva la forma de mover el apuntador.  Ahora IX apunta al cuarto elemento d Z (Z[3]).
    Y = *IX;            // Y se modifica, ahora vale Z[3] (8).
    printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z [4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);

    IX = IX + 4;        // IX se modifica. Observa que el apuntador se mueve 4 posiciones y cae en una direccion afuera del arreglo. Esto ocacionara un error.
    Y = *IX;            // Y se modifica, toma el calor (basura) de una celda incorrecta. Es un error que no señala el compilador del lenguaje C.
    printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z [4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
